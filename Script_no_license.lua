...
local function getServers()
    ...
    return { "http://127.0.0.1:8080/licenses.txt" }
end
...
local function loadSavedLicense()
    -- Always behave as if no saved license exists (developer/test mode)
    return nil
end
...
local function getUserType(licenseKey)
    -- Return premium for testing
    return string.char(112,114,101,109,105,117,109)
end
...
local function checkOnlineLicense(licenseKey, currentDeviceId)
    -- Always valid in developer/test mode
    return true, string.char(68,69,86,73,67,69,95,77,65,84,67,72), 1, string.char(112,114,101,109,105,117,109)
end
...
local function checkAutoLogin()
    -- Skip online checks and always return premium user
    return true, string.char(112,114,101,109,105,117,109)
end
...
local function enhancedLoginSystem()
    -- Developer/test mode: do not prompt for license, enable all features.
    return true, string.char(112,114,101,109,105,117,109)
end
...
local function checkForUpdates()
    -- Disable update checks in developer/test mode.
    return false
end
...
local function mandatoryUpdateCheck()
    -- Disable mandatory update checks in developer/test mode.
    return false
end
...
local function checkFunctionsControl()
    -- In developer/test mode, enable all features locally without contacting server.
    functionsControl.lastCheck = c.time() * 1000
    functionsControl.enabled = true
    for k,_ in pairs(functionsControl.features) do
        functionsControl.features[k] = true
    end
    functionsControl.messages.maintenance = string.char(34,34)
    functionsControl.messages.announcement = string.char(34,34)
    return true
end
...
local function getFeatureStatus(featureName)
    -- In developer/test mode, allow all features.
    return true
end
